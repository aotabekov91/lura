...
class PageItem(QGraphicsObject):
    ...
    def __init__(self, page, index, settings, view):
        ...
        self.m_view.window.annotationAdded.connect(self.on_annotationChanged)

    def on_annotationChanged(self, page):
        if not self.isVisible(): return
        if self.view().id()==page.pageItem().view().id(): return
        if page.document().hash()!=self.page().document().hash(): return
        if page.pageNumber()!=self.page().pageNumber(): return
        print(self.view().id(), page.pageItem().view().id())
        print(page.document(), self.page().document())
        self.refresh(dropCachedPixmap=True)
    ...
    # def paintHighlight(self, painter, optins, widgets):
