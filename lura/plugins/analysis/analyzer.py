...
class Analyzer:
    ...
    def parse(self, filePath):
        ...
        if not filePath in self.documents:
            print(filePath)
            self.documents[filePath]=pipeline(filePath) 
            print(self.documents)
            raise
        return self.documents[filePath]
    
    def getSections(self, filePath):
        df=self.parse(filePath)
        return df[df.type=='Section'].T.to_dict()

    def getHeaders(self, filePath):
        df=self.parse(filePath)
        return df[df.type=='Header'].T.to_dict()

    def getAuthors(self, filePath):
        df=self.parse(filePath)
        return df[df.type=='Author'].T.to_dict()

    def getParagraphs(self, filePath):
        df=self.parse(filePath)
        paragraphs=df[df.type=='Paragraph'].T.to_dict()
        for d, p in paragraphs.items():
            p['keywords']=keywords.keywords(p['text']).split('\n')
        return paragraphs

    def getSummary(self, filePath, words=100):
        df=self.parse(filePath)
        txt=' '.join(df['text'].values())
        return summarizer.summarize(txt, words)
        
    def getBibliographys(self, filePath):
        df=self.parse(filePath)
        return df[df.type=='Bibliography'].T.to_dict()
