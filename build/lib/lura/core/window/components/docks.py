...
class Docks(QWidget):
    ...
    def createDocks(self):
        ...
        for name, loc in locs.items():
            ...
            stackWidget.setFixedHeight(250)
            if name=='right':
    ...
    def setActions(self):
        self.actions=Actions(self.window.app, 'Docks', self)

    @classify(parent='window', context=Qt.WindowShortcut)
    def toggle(self):
        if not self.setFocus():
            statusbar=self.window.statusBar()
            statusbar.setDetailInfo(self.actions.name)
            statusbar.show()
            self.show()
            self.setFocus()
        else:
    ...
    @classify(parent='own', context=Qt.WidgetShortcut)
