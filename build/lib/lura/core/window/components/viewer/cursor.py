...
class Cursor(QObject):
    ...
    def __init__(self, view): 
        ...
        self.view.mousePressEventOccured.connect(self.on_mousePress)
        self.view.mouseMoveEventOccured.connect(self.on_mouseMove)
        self.view.mouseReleaseEventOccured.connect(self.on_mouseRelease)
        self.view.mouseDoubleClickOccured.connect(self.on_doubleClick)
        self.view.pageHasBeenJustPainted.connect(self.paint_selection)
        self.view.keyPressEventOccurred.connect(self.on_keyPressEvent)
    ...
    def update_move(self, event, pageItem):
        ...
        for r in rect:
            text=pageItem.page().find(r)
            text=text.split('\n')
            for t in text:
                tRectF=pageItem.page().search(t)
                for i in tRectF:
                    if i.intersects(r):
    ...
    def paint_selection(self, painter, options, widgets, pageItem, view):
        if len(self.selected_area)>0:
    ...
    def on_doubleClick(self, event, pageItem):
        ...
        for word in text.split(' '):
            tRectF=pageItem.page().search(word)
            for i in tRectF:
                if i.contains(point):
    ...
    def on_keyPressEvent(self, event):
        if event.key()==Qt.Key_Escape and self.selected_area:
            self.clear()
    ...
    def clear(self):
        ...
        self.selected_text=[]
        self.m_rubberBand=None
        self.view.pageItem().update()

    def get_selection(self, clear=False):
        t, a= self.selected_text, self.selected_area
        if clear: self.clear()
        return t, a
